...
interface QueryDao {
    ...
    @Query("SELECT team.name FROM team " +
            " JOIN color ON team.color_id = color.color_id WHERE color.name = :uniform_color")
    ...
    @Query("SELECT first_name, last_name FROM player " +
            "JOIN team ON player.team_id = team.team_id WHERE team.name = :team ORDER BY :ppg DESC")
    fun query3(team: String, ppg: String = Constants.PPG): Flow<List<PlayerName>>
    ...
    @Query("SELECT uniform_num, first_name, last_name FROM player " +
    ...
    @Query("SELECT first_name, last_name, team.name, wins FROM player " +
            "JOIN team ON player.team_id = team.team_id " +
            "WHERE wins > :num_wins")
